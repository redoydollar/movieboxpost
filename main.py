...
def set_state(user_id, action, temp_data=None):
    states_col.update_one(
        {"user_id": user_id},
        {"$set": {"action": action, "temp_data": temp_data or {}}},
        upsert=True
    )
...
def ad_timer(chat_id, msg_id, next_markup):
    ...
    time.sleep(duration)
    try:
        bot.edit_message_reply_markup(chat_id, msg_id, reply_markup=next_markup)
    except: pass
...
def search_content(message):
    ...
    if state: return # Admin workflow e thakle search hobe na
    ...
    buttons = []
    for m in movies:
        buttons.append([telebot.types.InlineKeyboardButton(f"🎬 {m['title']}", callback_data=f"mov_{m['_id']}")])
    for s in series:
        buttons.append([telebot.types.InlineKeyboardButton(f"📺 {s['title']}", callback_data=f"ser_{s['_id']}")])
...
def callback_query(call):
    ...
    if data == "admin_add_movie":
        ...
        if not is_admin(user_id): return
        mapping = {
            "admin_set_ad1": "set_ad_normal_ad1", 
            "admin_set_ad2": "set_ad_normal_ad2",
            "admin_set_aad1": "set_ad_adult_ad1", 
            "admin_set_aad2": "set_ad_adult_ad2"
        }
    ...
    # TMDB Movie Selection
        ...
        if poster_path:
            ...
            bot.send_message(user_id, "No poster found on TMDB. Send Poster Image manually:")
    ...
    # TMDB TV Selection
        ...
        temp_data = {
            ...
            "series_id": "" # Will be created after saving
        }
        
        poster_path = details.get('poster_path')
        if poster_path:
            poster_url = f"https://image.tmdb.org/t/p/w500{poster_path}"
            msg = bot.send_photo(user_id, poster_url, caption=f"📺 Selected: *{temp_data['title']}*\n\nSeries saved! Now use /addepisode to add episodes.", parse_mode="Markdown")
            temp_data['poster_file_id'] = msg.photo[-1].file_id
        else:
            temp_data['poster_file_id'] = ""
            
        # Directly save series to DB
        series_insert = series_col.insert_one({
            "title": temp_data['title'], 
            "poster_file_id": temp_data['poster_file_id'],
    ...
    # ============ USER CALLBACKS ============
        ...
        buttons = telebot.types.InlineKeyboardMarkup([
            [telebot.types.InlineKeyboardButton("📥 Download File", callback_data=f"dl_mov_{movie['_id']}")]
        ])
        ...
        for ep in series.get('episodes', []):
            buttons.append([telebot.types.InlineKeyboardButton(f"Episode {ep['ep_num']} - {ep['name']}", callback_data=f"dl_ep_{series['_id']}_{ep['ep_num']}")])
        ...
        if not ad1 or not ad2:
            bot.send_message(user_id, "⚠️ Ads not set by admin. Use /admin to set.")
            return
...
def deliver_file(user_id, data):
    try:
        ...
        bot.send_message(user_id, "❌ Error fetching file.")
...
def handle_state(message):
    ...
    try:
        ...
        if action == 'wait_tmdb_query':
            ...
            if not results:
                bot.reply_to(user_id, "❌ Not found. Send another name:")
                return
            markup = telebot.types.InlineKeyboardMarkup()
            for r in results:
                title = r.get('title', 'N/A')
                year = r.get('release_date', '')[:4]
                markup.add(telebot.types.InlineKeyboardButton(f"{title} ({year})", callback_data=f"tmdb_{r['id']}"))
        ...
        # --- TMDB TV Search ---
            ...
            for r in results:
                ...
                markup.add(telebot.types.InlineKeyboardButton(f"{name} ({year})", callback_data=f"tmdbtv_{r['id']}"))
            bot.send_message(user_id, "Select a series:", reply_markup=markup)
            clear_state(user_id)

        # --- ADD MOVIE FLOW (After TMDB Selection) ---
        elif action == 'add_mov_ss':
            sids = []
            if message.photo: sids = [p.file_id for p in message.photo]
            temp['screenshots'] = sids
            set_state(user_id, 'add_mov_file', temp)
            ...
            clear_state(user_id)
            bot.reply_to(user_id, "✅ Movie Added Successfully with TMDB Data!")

        # --- ADD EPISODE FLOW ---
        elif action == 'add_ep_name':
            temp['ep_name'] = message.text
            set_state(user_id, 'add_ep_file', temp)
            ...
            series = series_col.find_one({"_id": ObjectId(temp['series_id'])})
            ep_num = len(series['episodes']) + 1
            
            series_col.update_one(
                {"_id": ObjectId(temp['series_id'])},
                {"$push": {"episodes": {"ep_num": ep_num, "name": temp['ep_name'], "file_id": file_id}}}
            )
            clear_state(user_id)
            bot.reply_to(user_id, f"✅ Episode {ep_num} added to {series['title']}")
        ...
        # --- AD SETTINGS FLOW ---
            ...
            ads_col.update_one({}, {"$set": {ad_key: message.photo[-1].file_id}})
            clear_state(user_id)
            bot.reply_to(user_id, f"✅ Ad ({ad_key}) Updated Successfully!")

    except Exception as e:
        clear_state(user_id)
        bot.reply_to(user_id, f"❌ Error: {e}. Process cancelled.")

if __name__ == "__main__":
    print("🤖 Bot is running with TMDB API...")
